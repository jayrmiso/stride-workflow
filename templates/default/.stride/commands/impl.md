...
```text
workers(default: stridebuilder + stridereviewer) -> worktree -> load spec -> light probe -> builder worker -> checker -> debugger if needed -> ui-auditor if visual -> reviewer worker -> fixer via builder if needed -> checker again -> previewer if user-facing -> handoff -> ledger -> final report
```
